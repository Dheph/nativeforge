...
program2.name("nativeforge").description("The ultimate CLI for scaffolding React Native and Expo architectures.").version(version);
program2.command("init").description("Initialize a new NativeForge project").action(async () => {
  oe(`Welcome to NativeForge v${version}`);
  const projectType = await ie({
  ...
  $e(`Successfully initialized ${String(projectName)} with ${String(projectType)}! (Dummy output)`);
});
program2.parse();
